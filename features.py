...
def get_time_5_feet(feature_df, tracking, locations):
    ...
    # Now, we count only the consecutive frames where the player is within 5 feet before they move away
    def count_consecutive_within_5_feet(group):
        consecutive_count = 0
        for within_5 in group['within_5_feet']:
            if within_5:
                consecutive_count += 1
            else:
                break  # Stop counting when the player moves out of the 5 feet range
        return consecutive_count
    
   # Apply the consecutive counting function for each play
    play_df = tracking_df.groupby('play_iid').apply(count_consecutive_within_5_feet).reset_index(name='continuous_5_feet_frames')
